...
class PostureOptimizerDebug(Node):
    """
    Extends the geometric feasibility tester with an Artificial Potential Field (APF)
    planner. After computing per-cell clearances and optimal postures, the APF:

      1. Builds a scalar potential field over the safe navigable grid:
            U(x,y) = U_attr(x,y)  +  U_rep(x,y)
         - U_attr  : parabolic well pulling toward the goal
         - U_rep   : wall repulsion derived directly from the w_avail EDT field

      2. Runs gradient descent from the robot's current position to the goal,
         producing a 1-D ordered path of (col, row) grid indices.

      3. Samples opt_z along that path and applies a 1-D Gaussian smooth,
         yielding a smooth body-height trajectory the robot can follow.

    All intermediate fields and the final path are published as PointCloud2
    topics for inspection in RViz.
    """

    # ------------------------------------------------------------------ #
    #  APF tuning constants                                               #
    # ------------------------------------------------------------------ #
    K_ATTR      = 1.0    # Attractive gain  (higher → pulls harder toward goal)
    K_REP       = 0.08   # Repulsive gain   (higher → stays further from walls)
    D0          = 0.6    # Influence radius of repulsive field (meters)
                         # Cells with w_avail > D0 feel zero repulsion
    GRAD_STEP   = 0.3    # Fraction of one cell to step per gradient iteration
                         # 0.5–0.9 is stable; lower = smoother but more steps
    MAX_ITER    = 2000   # Safety cap on gradient descent iterations
    CONVERGE_M  = 0.06   # Stop when within this many meters of the goal
    HEIGHT_SIGMA = 2.5   # Gaussian σ (in cells) for height trajectory smoothing
                         # Higher → smoother height changes, less reactive
    ...
    def __init__(self):
        ...
        # Flat heatmap of the combined potential — lets you see the "bowl"
        self.pub_apf_path  = self.create_publisher(PointCloud2, '/apf_path_pc',         10)
        # The actual planned path (3-D: floats at smooth body-height above floor)
        ...
        # ── Goal point (world frame, meters) ──────────────────────────
        # Change these to wherever you want the robot to navigate to.
        # Declare as ROS parameters so you can set them at launch:
        #   ros2 run <pkg> map2sdf --ros-args -p goal_x:=2.0 -p goal_y:=1.5
    ...
    def world_to_grid(self, wx: float, wy: float, info) -> tuple[int, int]:
        """
        Convert a world-frame (x, y) coordinate into (col, row) grid indices.
        Mirrors the inverse of the index→world mapping used in publish_pointcloud.

        Grid layout (from elevation_mapping convention):
            col 0 = top of array  = max world-Y
            row 0 = left of array = max world-X
        """
        ...
        # Invert:  world_y = cy + half_y - (col + 0.5) * res
        col = int((cy + half_y - wy) / res - 0.5)
        # Invert:  world_x = cx + half_x - (row + 0.5) * res
        row = int((cx + half_x - wx) / res - 0.5)
        return col, row

    def grid_to_world(self, col: float, row: float, info) -> tuple[float, float]:
        """Continuous version — accepts float indices for sub-cell interpolation."""
        cx     = info.pose.position.x
        cy     = info.pose.position.y
        half_x = info.length_x / 2.0
        half_y = info.length_y / 2.0
    ...
    def compute_apf(
        ...
        """
        Build the combined APF potential field U = U_attr + U_rep over the grid.

        U_attr(c, r) = K_ATTR * [ (c - goal_c)² + (r - goal_r)² ] * res²
                     — a parabolic bowl centred on the goal (in meters²)

        U_rep(c, r)  = K_REP * (1/w - 1/D0)²   if w < D0
                     = 0                          otherwise
                     where w = w_avail in meters

        Cells outside safe_mask are set to +inf so gradient descent
        never steps into them.
        """
        rows, cols = safe_mask.shape          # note: numpy shape = (n_rows, n_cols)
        # Build index grids
        c_idx, r_idx = np.meshgrid(
            np.arange(safe_mask.shape[0]),    # col axis
            np.arange(safe_mask.shape[1]),    # row axis
            indexing='ij'
        )

        # ── Attractive term ───────────────────────────────────────────
        # Distance² from each cell to goal in grid units; scale by res² → meters²
        # Using grid units (not world coords) is fine because the grid is square.
        U_attr = self.K_ATTR * ((c_idx - goal_col)**2 + (r_idx - goal_row)**2).astype(np.float32)

        # ── Repulsive term ────────────────────────────────────────────
        # w_avail already in meters; NaN outside valid region
        ...
        # ── Combine ───────────────────────────────────────────────────
        U = U_attr + U_rep
        # Mask out non-navigable cells with +inf so gradient descent bounces off
    ...
    def gradient_descent_path(
        ...
        """
        Walk downhill on U from (start_col, start_row) toward (goal_col, goal_row).

        Uses a finite-difference gradient computed on the fly at each step.
        Returns a list of (col, row) float positions along the path.

        The step is taken as:
            [dc, dr] = -∇U / |∇U|  * GRAD_STEP
        (unit-normalised so step size is always exactly GRAD_STEP cells)
        """
        n_cols, n_rows = U.shape
        path = []

        # ── Snap start to nearest safe cell ───────────────────────────
        # The map centre may land on an unsafe (+inf) cell. Find the safe
        # cell with the lowest U value closest to the requested start.
        ...
        for _ in range(self.MAX_ITER):
            ...
            # Integer neighbours for finite-difference gradient
            ci = int(np.clip(round(c), 1, n_cols - 2))
            ri = int(np.clip(round(r), 1, n_rows - 2))

            # Central differences (∂U/∂col, ∂U/∂row)
            # Replace any inf neighbours with the centre value so that
            # inf - inf = NaN never occurs.
            U_centre = U[ci, ri]
            ...
            if not np.isfinite(grad_mag) or grad_mag < 1e-9:
                # Flat or degenerate — nudge directly toward goal
                dc   = goal_col - c
                dr   = goal_row - r
                ...
                # Step opposite to gradient (downhill)
    ...
    def sample_smooth_height(
        ...
        """
        For each waypoint along the gradient path, look up the optimal body height
        from opt_z (nearest-cell interpolation), then apply a 1-D Gaussian smooth.

        Returns
        -------
        heights_smooth : 1-D array of smoothed body heights (meters) per waypoint
        floor_heights  : 1-D array of floor elevation at each waypoint (for 3-D viz)
        """
        ...
        for (c, r) in path:
            ...
            # If this cell has no valid opt_z (NaN), inherit the last good value
        ...
        # 1-D Gaussian smooth along the path dimension
        heights_smooth = gaussian_filter1d(raw_heights, sigma=self.HEIGHT_SIGMA)

        # Hard-clamp back inside kinematic limits (smoothing can overshoot slightly)
    ...
    def publish_pointcloud(self, publisher, header, info, values, base_z, mask, is_flat=True):
        """Unchanged from original — publishes 2D heatmaps or 3D marker clouds."""
        if not np.any(mask):
            return
    ...
    def publish_apf_field(self, header, info, U: np.ndarray, safe_mask: np.ndarray):
        """
        Publish the APF potential field as a flat heatmap.
        Intensity = U value (lower = more attractive).
        Inf cells are excluded via safe_mask.
        """
        # Cap U for visualisation — very large values wash out the colour scale
    ...
    def publish_apf_path(
        ...
        """
        Publish the planned path as a 3-D PointCloud2 where each point floats
        at  floor_height + smooth_body_height / 2  (the robot's centre of mass).
        Intensity encodes the body height so RViz can colour by posture.
        """
        if not path:
            return
        ...
        for i, (c, r) in enumerate(path):
            ...
            h  = heights_smooth[i]
            f  = floor_heights[i]
            wz = f + h / 2.0          # centre of mass height
        ...
        self.pub_apf_path.publish(cloud_msg)
    ...
    def callback(self, floor_msg: GridMap, ceil_msg: GridMap):
        ...
        # ── Phase 1 & 2: existing geometry pipeline ───────────────────
        ...
        # Read goal from ROS parameters (can be changed at runtime via ros2 param set)
        ...
        # Robot start = centre of the current map (camera/sensor origin)
        ...
        # Gradient descent → waypoint list
