...
from std_msgs.msg import Float64 # NEW: For sending height to RL policy

from message_filters import ApproximateTimeSynchronizer, Subscriber
from scipy.ndimage import distance_transform_edt, gaussian_filter1d

class APFPlannerNode(Node):
    # --- APF Tuning (From map2apf3.py) ---
    K_ATTR       = 1.0    
    K_REP        = 0.08   
    D0           = 0.6    
    GRAD_STEP    = 0.7    
    ...
    # --- Kinematics (Target Height Range) ---
    Z_MIN = 0.04 # CLAMPED TO YOUR RL TRAINING RANGE
    Z_MAX = 0.11 # CLAMPED TO YOUR RL TRAINING RANGE
    S_MIN = 0.25   
    S_MAX = 0.50   
    ...
    def adam_optimizer_path(self, U, start_col, start_row, goal_col, goal_row, res):
        ...
        if not finite_mask.any(): return path
        ...
        for _ in range(self.MAX_ITER):
            ...
            c, r = float(np.clip(c, 0, n_cols - 1)), float(np.clip(r, 0, n_rows - 1))
    ...
    def sample_smooth_height(self, path, opt_z, floor):
        ...
        for (c, r) in path:
            ci, ri = int(np.clip(round(c), 0, n_cols - 1)), int(np.clip(round(r), 0, n_rows - 1))
            z, f = opt_z[ci, ri], floor[ci, ri]
            if np.isnan(z) and raw_heights: z = raw_heights[-1]
            elif np.isnan(z): z = self.Z_MIN
    ...
    def callback(self, floor_msg: GridMap, ceil_msg: GridMap):
        if 'elevation' not in floor_msg.layers or 'elevation' not in ceil_msg.layers: return
        ...
        if np.sum(safe_mask) < 5: return

        goal_wx, goal_wy = self.get_parameter('goal_x').value, self.get_parameter('goal_y').value
        ...
        # Start at map center (where the camera/robot is)
